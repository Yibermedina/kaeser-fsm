...
export default function FiltrosProgramacion({ anio, mes, tecnico, estado, tecnicos, anios, vista = 'mensual' }: {
  anio: number; mes: number; tecnico: string; estado: string;
  tecnicos: { id: string; nombre: string }[]; anios: number[]; vista?: 'mensual' | 'semanal';
}) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-[22px] border border-[#E4E7EB] bg-white p-3 shadow-sm">
      <div className="flex items-center gap-1 rounded-xl border border-[#E4E7EB] bg-[#f8f9fa] p-1">
        <button
          type="button"
          onClick={() => actualizar({ vista: 'mensual' })}
          className={`rounded-lg px-3 py-1.5 text-sm font-bold ${vista === 'mensual' ? 'bg-[#0C0C0C] text-[#FDC100]' : 'text-[#4B5563]'}`}
        >
          Mensual
        </button>
        <button
          type="button"
          onClick={() => actualizar({ vista: 'semanal' })}
          className={`rounded-lg px-3 py-1.5 text-sm font-bold ${vista === 'semanal' ? 'bg-[#0C0C0C] text-[#FDC100]' : 'text-[#4B5563]'}`}
        >
          Semanal por técnico
        </button>
      </div>
      ...
      {(tecnico || estado || vista !== 'mensual') && <button onClick={() => actualizar({ tecnico: '', estado: '', vista: 'mensual' })} className="rounded-xl px-3 py-2 text-sm font-bold text-[#6B7280] underline-offset-2 hover:underline">Limpiar</button>}
